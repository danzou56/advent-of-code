...
internal class Day12 : AdventTestRunner24("Garden Groups") {
  ...
  tailrec fun divideRegions(
    ...
    val curPlot =
        dfs(start) {
          garden.neighboringPos(it).filter { garden[it] == target }.toSet()
        }
    ...
    val plots =
        garden.indices2D
            ...
            .flatMap { (plotChar, plotPos) ->
              divideRegions(garden, plotPos.toSet()).map {
                plotChar to it
              }
            }

    return plots
        .map { (plotChar, posList) ->
  ...
  override fun part2(input: String): Any {
    ...
    val plots =
        garden.indices2D
            ...
            .flatMap { (plotChar, posList) ->
              divideRegions(garden, posList.toSet()).map {
                plotChar to it
              }
            }
    ...
    return plots.sumOf { (plotChar, posList) ->
      ...
      val sides =
          posList
              // Find all perimeter positions - since we need to find "sides", we also need to keep
              ...
              // coordinate for north/south sides and share the same x coordinate for east/west
              // sides
              .groupBy { (pos, dir) -> if (dir.x == 0) pos.y to dir else pos.x to dir }
              .values
              .sumOf { posDirList: List<Pair<Pos, Pos>> ->
                // At this point, the direction the side faces is no longer relevant and can be
                // thrown away
                val posSet = posDirList.map { (pos, _) -> pos }.toSet()
                // Grouping by pos.y/x,dir will produce scenarios where disjointed sides are in the
